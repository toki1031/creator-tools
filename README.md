# Creator OS Sprint 3.6.0 — シーン別ナレーション＆字幕同期
...
## Sprint 3.6.0 追加 — フレーズ字幕同期
- 1シーン内の字幕を句読点と文字数で短いフレーズへ自動分割
- シーン別WAVの実尺に対して文字量＋句読点の重みで表示時間を自動配分
- ナレーション進行に合わせて字幕フレーズを順番に切替
- 短すぎる断片は前フレーズへ結合してちらつきを抑制
- `scene.subtitlePhraseSync=false` で従来の1シーン固定字幕へ戻せる

注意: Kokoroから単語単位タイムスタンプは取得していないため、現段階はWAV実尺を基準にした推定同期です。音声そのものを分割しないため、読み上げの自然さは維持します。
