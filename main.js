...
async function renderOutput(id) {
  ...
  const describeAssets=value=>{const imageText=`画像 ${value.loadedImageCount ?? value.images.filter(Boolean).length}/${scenes.length}`;const imageWarning=value.imageFailures?.length?`（読込失敗 ${value.imageFailures.length}件）`:'';const audioText=o.bgmEnabled&&project.bgm?.audioData?(value.audioInvalid?'BGMが動画ファイルです':value.audioFetchError?'BGMファイル読込失敗':'BGMファイル確認済み'):'BGMなし';diagnostics.textContent=`${imageText}${imageWarning}／${audioText}`;diagnostics.classList.toggle('warn',Boolean(value.imageFailures?.length||value.audioFetchError||value.audioInvalid));};
  ...
  root.querySelector('#generateVideo').onclick=async()=>{
    ...
    try{
      ...
      await persistSettings();let assets=await preparedPromise;if(project.output?.bgmEnabled&&project.bgm?.audioData&&!assets.audioArrayBuffer){assets=await prepareVideoProject(project,{onStatus:text=>renderStatus.textContent=text});prepared=assets;describeAssets(assets);}
