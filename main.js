...
loopEl.onblur=()=>commitBgmLoopDecision(loopEl);
  ...
  const subtitleTextColorBeforeByElement=new WeakMap();
...
let textColorOutsideCommitArmed=false;
const rememberSubtitleTextColorBefore=el=>{if(subtitleTextColorBeforeByElement.has(el))return;subtitleTextColorBeforeByElement.set(el,snapshotSubtitleTextColor(st.textColor));};
...
const armSubtitleTextColorOutsideCommit=()=>{if(textColorOutsideCommitArmed)return;textColorOutsideCommitArmed=true;document.addEventListener('pointerdown',()=>{textColorOutsideCommitArmed=false;commitSubtitleTextColorDecision(textColorEl);},{capture:true,once:true});};
...
textColorEl.oninput=()=>{rememberSubtitleTextColorBefore(textColorEl);armSubtitleTextColorOutsideCommit();readGlobalSettings();updateLabels();renderSubtitleEditor();renderSubtitlePreview();save();scheduleSubtitleTextColorCloseCheck(textColorEl);};
...
const subtitleOutlineColorBeforeByElement=new WeakMap();
...
const subtitleBackgroundColorBeforeByElement=new WeakMap();
...
backgroundColorEl.onblur=()=>{if(isColorPickerOpen(backgroundColorEl))return;commitSubtitleBackgroundColorDecision(backgroundColorEl);};
  const subtitleEnabledBeforeByElement=new WeakMap();
...
async function renderOutput(id) {
  ...
  let renderController=null,resultUrl='';let resultFile=null;let generatedHash='';let generatedMeta=null;
  ...
  root.querySelector('#generateVideo').onclick=async()=>{
    ...
    renderController=new AbortController();root.querySelector('#cancelRender').disabled=false;root.querySelector('#generateVideo').disabled=true;progress.value=0;
    try{
