...
async function renderProject(id) {
  ...
  root.innerHTML = `
    <main class="shell editor-shell">
      ...
      <section class="editor-card"><div class="section-head"><div><h2>動画用ナレーション</h2><p>まずは音声ファイルを動画へ合成します。将来は無料Voice Libraryの生成音声もここへ自動接続します。</p></div><span id="narrationFileStatus">${project.narration?.audioData?'登録済み':'未登録'}</span></div><label>ナレーション音声<input id="narrationFile" type="file" accept="audio/*,.mp3,.m4a,.aac,.wav"><small id="narrationFileName">${escapeHtml(project.narration?.fileName||'未登録')}</small><small>MP3 / M4A / AAC / WAV対応。MOV / MP4などの動画ファイルは使用できません。</small></label><audio id="narrationPreview" controls ${project.narration?.audioData?`src="${project.narration.audioData}"`:''}></audio><div class="form-grid"><label>ナレーション音量<div class="range-line"><input id="narrationVolume" type="range" min="0" max="1" step="0.01" value="${project.narration?.volume ?? 1}"><span id="narrationVolumeValue">${Math.round((project.narration?.volume ?? 1)*100)}%</span></div></label></div><div class="tool-row"><button id="removeNarration" class="danger" ${project.narration?.audioData?'':'disabled'}>ナレーション音声を外す</button></div></section>
  ...
  const narrationFileInput=root.querySelector("#narrationFile");
  const narrationVolume=root.querySelector("#narrationVolume");
  const narrationVolumeValue=root.querySelector("#narrationVolumeValue");
  narrationVolume.oninput=()=>{narrationVolumeValue.textContent=`${Math.round(Number(narrationVolume.value)*100)}%`;scheduleSave();};
  narrationFileInput.onchange=async()=>{const file=narrationFileInput.files?.[0];if(!file)return;const type=String(file.type||'').toLowerCase();const name=String(file.name||'').toLowerCase();if(type.startsWith('video/')||/\.(mov|mp4|m4v|avi|webm)$/.test(name)){narrationFileInput.value='';return alert('選択したファイルは動画です。ナレーションにはMP3・M4A・AAC・WAVなどの音声ファイルを選択してください。');}if(type&&!type.startsWith('audio/')&&!/\.(mp3|m4a|aac|wav)$/.test(name)){narrationFileInput.value='';return alert('対応していない音声形式です。MP3・M4A・AAC・WAVを選択してください。');}project.narration.audioData=await fileToDataUrl(file);project.narration.fileName=file.name;project.narration.mimeType=file.type||'';project.narration.source='upload';root.querySelector('#narrationFileName').textContent=file.name;root.querySelector('#narrationFileStatus').textContent='登録済み';root.querySelector('#narrationPreview').src=project.narration.audioData;root.querySelector('#removeNarration').disabled=false;project.updatedAt=new Date().toISOString();await saveProject(project);};
  root.querySelector('#removeNarration').onclick=async()=>{project.narration.audioData='';project.narration.fileName='';project.narration.mimeType='';project.narration.source='browser';root.querySelector('#narrationFileName').textContent='未登録';root.querySelector('#narrationFileStatus').textContent='未登録';root.querySelector('#narrationPreview').removeAttribute('src');root.querySelector('#narrationPreview').load();root.querySelector('#removeNarration').disabled=true;project.updatedAt=new Date().toISOString();await saveProject(project);};
...
async function renderOutput(id) {
  ...
  const describeAssets=value=>{const imageText=`画像 ${value.loadedImageCount ?? value.images.filter(Boolean).length}/${scenes.length}`;const imageWarning=value.imageFailures?.length?`（読込失敗 ${value.imageFailures.length}件）`:'';const audioText=o.bgmEnabled&&project.bgm?.audioData?(value.audioInvalid?'BGMが動画ファイルです':value.audioFetchError?'BGMファイル読込失敗':'BGMファイル確認済み'):'BGMなし';const narrationText=project.narration?.audioData?(value.narrationInvalid?'ナレーションが動画ファイルです':value.narrationFetchError?'ナレーション読込失敗':'ナレーション確認済み'):'ナレーションなし';diagnostics.textContent=`${imageText}${imageWarning}／${audioText}／${narrationText}`;diagnostics.classList.toggle('warn',Boolean(value.imageFailures?.length||value.audioFetchError||value.audioInvalid||value.narrationFetchError||value.narrationInvalid));};
  ...
  root.querySelector('#generateVideo').onclick=async()=>{
    ...
    try{
      if((project.output?.bgmEnabled&&project.bgm?.audioData)||project.narration?.audioData){const AudioContextClass=globalThis.AudioContext||globalThis.webkitAudioContext;if(AudioContextClass){unlockedAudioContext=new AudioContextClass();void unlockedAudioContext.resume();}}
      await persistSettings();let assets=await preparedPromise;if((project.output?.bgmEnabled&&project.bgm?.audioData&&!assets.audioArrayBuffer)||(project.narration?.audioData&&!assets.narrationArrayBuffer)){assets=await prepareVideoProject(project,{onStatus:text=>renderStatus.textContent=text});prepared=assets;describeAssets(assets);}
