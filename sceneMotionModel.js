...
export function sceneMotionFeatures(context = {}) {
  const features = [];
...
export function trainSceneMotionModel(examples = [], { alpha = 1 } = {}) {
  ...
  for (const example of Array.isArray(examples) ? examples : []) {
    ...
    const tokens = sceneMotionFeatures(example?.context);
...
export function predictSceneMotion(model, context = {}) {
  ...
  const features = sceneMotionFeatures(context);
  const scores = [];
