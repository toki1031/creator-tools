...
if (route.page !== 'scenes' || !route.id) {
  // Optional module: no-op outside Scene editing.
} else {
  void mountSmartReframe(route.id);
}
...
async function mountSmartReframe(projectId) {
  ...
  dialog.innerHTML = `
    ...
    <div class="tool-row" style="flex-wrap:wrap">
      <button type="button" data-reframe-preset="left">左を主役</button>
      <button type="button" data-reframe-preset="center">中央</button>
      <button type="button" data-reframe-preset="right">右を主役</button>
      <button type="button" data-reframe-preset="top">上寄せ</button>
    </div>
  ...
  function sceneForIndex(index) {
    return Array.isArray(project?.scenes) ? project.scenes[index] : null;
  }
  ...
  async function analyzeImage(src) {
    ...
    for (let y = 1; y < size - 1; y++) {
      for (let x = 1; x < size - 1; x++) {
        ...
        const gy = Math.abs(luminance[i + size] - luminance[i - size]);
        saliency[i] = gx + gy;
      }
    }
  ...
  function injectButtons() {
    ...
    cards.forEach((card, index) => {
      ...
      if (!controls) return;
      const existing = controls.querySelector('[data-smart-reframe]');
      if (existing) { existing.textContent = scene?.smartReframe ? 'Smart Reframe ✓' : 'Smart Reframe'; return; }
  ...
  dialog.querySelectorAll('[data-reframe-preset]').forEach(button => {
    button.addEventListener('click', () => {
      const preset = button.dataset.reframePreset;
      const presets = {
        left: { focusX: 0.3, focusY: 0.5, zoom: 1.08 },
        center: { focusX: 0.5, focusY: 0.5, zoom: 1 },
        right: { focusX: 0.7, focusY: 0.5, zoom: 1.08 },
        top: { focusX: 0.5, focusY: 0.35, zoom: 1.08 }
      };
      candidate = presets[preset] || presets.center;
      renderCandidate();
