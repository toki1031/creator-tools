...
test('separates timing, narration, subtitle and visual instructions', () => {
  ...
  assert.equal(first.endSec, 4.52);
...
test('does not turn unlabeled visual instructions into speech', () => {
  const result = createAutoProductionProject({
    requestText: `Scene 1: 現代の会議を見せる。\n目的: 問題提起`,
    title: 'legacy visual only'
  });
  assert.equal(result.ok, true);
  ...
  assert.equal(result.project.scenes[0].productionDirection.visualDirection, '現代の会議を見せる。');
});
