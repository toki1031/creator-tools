...
import {MVP_SHORTS_SPEC,assessMvpVideoResult,describeVideoExportFailure,isMvpShortsProject,validateMvpShortsOutput} from '../videoMvp.js';
...
test('解像度・fps・3分超過を検出する',()=>{const p=make();p.output.width=720;p.output.height=1280;p.output.fps=60;const r=validateMvpShortsOutput(p,181);assert.equal(r.pass,false);assert.equal(r.errors.length,3);assert.ok(r.errors.some(x=>x.includes('180秒')));});
...
test('メモリ失敗時に次操作を返す',()=>{const t=describeVideoExportFailure(new Error('Out of memory'),{durationSec:68,width:1080,height:1920,fps:30});assert.match(t,/メモリ/);assert.match(t,/先頭10秒/);});
