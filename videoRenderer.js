...
function splitSubtitlePhrases(text, maxChars = 13) {
  ...
  // 短い読点単位は結合。maxCharsを少し超えても意味の区切りを優先する。
  const semantic = [];
  ...
  let phrases = semantic.flatMap(splitLong).map(x=>x.trim()).filter(Boolean);

  // 3文字以下などの不自然な孤立断片を隣へ吸収する。
  const merged = [];
